...
router.get('/getAllProperties', async (req, res) => {
  try {
    ...
    propertiesWithDetails.forEach(property => {
      ...
      console.log(`id: ${typeof property.id}`);
  ...
  router.put('/updateProperty/:id', validateToken(['Broker']), propUpload.array('images', 5), async (req, res) => {
  ...
  router.delete('/deleteProperty/:id',validateToken(['Broker']), async (req, res) => {
    const propertyId = req.params.id;
